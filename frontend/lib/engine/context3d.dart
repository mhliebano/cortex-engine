...
class Ray3D {
  ...
  const Ray3D({
    required this.ox, required this.oy, required this.oz,
    required this.dx, required this.dy, required this.dz,
  });
}
...
class RayHit {
  ...
  const RayHit({
    ...
    required this.px, required this.py, required this.pz,
...
class Context3D {
  ...
  void updateCamera([int mode = CameraMode.orbital]) {
    ...
    camera.posX = r.$1; camera.posY = r.$2; camera.posZ = r.$3;
    camera.targetX = r.$4; camera.targetY = r.$5; camera.targetZ = r.$6;
  }
  ...
  void drawBoxWires(double x, double y, double z, double w, double h, double d, [
    ColorRGBA color = ColorRGBA.white,
  ]) =>
      _g3d.drawBoxWires(x, y, z, w, h, d, color.r, color.g, color.b, color.a);

  void drawLine3D(double sx, double sy, double sz, double ex, double ey, double ez, [
    ColorRGBA color = ColorRGBA.white,
  ]) =>
      _g3d.drawLine3D(sx, sy, sz, ex, ey, ez, color.r, color.g, color.b, color.a);
  ...
  void drawSphere(double x, double y, double z, double radius, {
    ...
    _g3d.drawSphereEx(x, y, z, radius, rings, slices, color.r, color.g, color.b, color.a);
    if (wireColor != null) {
      _g3d.drawSphereWires(x, y, z, radius, rings, slices, wireColor.r, wireColor.g, wireColor.b, wireColor.a);
    }
  }

  void drawSphereWires(double x, double y, double z, double radius, {
    ColorRGBA color = ColorRGBA.white,
    int rings = 16,
    int slices = 16,
  }) =>
      _g3d.drawSphereWires(x, y, z, radius, rings, slices, color.r, color.g, color.b, color.a);
  ...
  void drawCylinder(double x, double y, double z, double radius, double height, {
    ...
    _g3d.drawCylinder(x, y, z, radius, height, slices, color.r, color.g, color.b, color.a);
    if (wireColor != null) {
      _g3d.drawCylinderWires(x, y, z, radius, height, slices, wireColor.r, wireColor.g, wireColor.b, wireColor.a);
    }
  }
  ...
  void drawCylinderEx(
    ...
    _g3d.drawCylinderEx(sx, sy, sz, ex, ey, ez, radiusStart, radiusEnd, slices,
        color.r, color.g, color.b, color.a);
    if (wireColor != null) {
      _g3d.drawCylinderWiresEx(sx, sy, sz, ex, ey, ez, radiusStart, radiusEnd, slices,
          wireColor.r, wireColor.g, wireColor.b, wireColor.a);
    }
  }

  /// Dibuja un plano horizontal centrado en (x, y, z).
  void drawPlane(double x, double y, double z, double width, double length, [
    ColorRGBA color = ColorRGBA.darkGray,
  ]) =>
      _g3d.drawPlane(x, y, z, width, length, color.r, color.g, color.b, color.a);
  ...
  void drawModel(int id, double x, double y, double z, {
    double scale = 1.0,
  ...
  void drawModelEx(int id, double x, double y, double z, {
    double axisX = 0, double axisY = 1, double axisZ = 0,
    double angle = 0,
    double scaleX = 1, double scaleY = 1, double scaleZ = 1,
    ColorRGBA tint = ColorRGBA.white,
  }) =>
      _g3d.modelDrawEx(id, x, y, z, axisX, axisY, axisZ, angle, scaleX, scaleY, scaleZ,
  ...
  RayHit? rayHitsModelMesh(Ray3D ray, int modelId, int meshIndex, List<double> transform) {
    assert(transform.length == 16, 'La matriz de transformación debe tener 16 elementos');
  ...
  void drawBoundingBox(BBox3D box, [ColorRGBA color = ColorRGBA.white]) =>
      _g3d.drawBbox(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ,
          color.r, color.g, color.b, color.a);
  ...
  void setUniformVec4(int shaderId, int loc, double x, double y, double z, double w) =>
  ...
  void setUniformMatrix(int shaderId, int loc, List<double> m) {
    ...
    _g3d.shaderSetMatrix(shaderId, loc,
  ...
  bool setModelTexture(int modelId, int textureId, {
    int materialIndex = 0,
  ...
  (int animSetId, int count) loadAnimations(String path) =>
      _g3d.animLoad(path);
  ...
  bool updateAnimation(int modelId, int animSetId, int animIndex, double frame) =>

...
class SplitButton extends Element {
  ...
  SplitButton({
    ...
    int width = 0,
    int height = 0,
  }) : super(width: width != 0 ? width : 160, height: height != 0 ? height : 40) {
  ...
  void onUpdate(double dt, InputEngine input) {
    ...
    if (_isDropdownOpen) {
      ...
      } else if (input.isMouseButtonPressed(MouseButtons.left) && !_isArrowHovered) {
        _isDropdownOpen = false;
      }
    }
  }
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    if (_isMainHovered) {
      ctx2d.drawRoundRect(x, y, mainW, height, 0.4, color: const ColorRGBA(255, 255, 255, 25));
    }

    // Resaltado de la zona de flecha desplegable
    if (_isArrowHovered || _isDropdownOpen) {
      ctx2d.drawRoundRect(x + mainW, y, arrowW, height, 0.4, color: const ColorRGBA(255, 255, 255, 40));
    }

    // Línea divisora
    ctx2d.drawRect(x + mainW, y + 6, 1, height - 12, const ColorRGBA(255, 255, 255, 80));
  ...
  void onRenderOverlay(Context2D ctx2d, Context3D ctx3d) {
    ...
    ctx2d.drawRoundRect(menuX, menuY, menuWidth, menuHeight, 0.2, color: const ColorRGBA(32, 38, 48));
    ctx2d.drawRoundRectLines(menuX, menuY, menuWidth, menuHeight, 0.2, lineThick: 1.0, color: const ColorRGBA(75, 85, 100));
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      if (isHovered) {
        ctx2d.drawRect(menuX + 4, itemY, menuWidth - 8, 34, const ColorRGBA(41, 128, 185));
      }

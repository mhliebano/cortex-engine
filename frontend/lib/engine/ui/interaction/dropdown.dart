...
class Dropdown<T> extends Element {
  ...
  void _scrollToSelected(int visibleMenuHeight) {
    ...
    if (index >= 0) {
      ...
      final maxScroll = totalContentHeight > visibleMenuHeight ? (totalContentHeight - visibleMenuHeight).toDouble() : 0.0;
      final targetOffset = (index * itemHeight) - (visibleMenuHeight / 2) + (itemHeight / 2);
  ...
  void onUpdate(double dt, InputEngine input) {
    ...
    final itemHeight = 36;
    final totalContentHeight = options.length * itemHeight;
    final visibleMenuHeight = totalContentHeight > maxMenuHeight ? maxMenuHeight : totalContentHeight;
    final maxScroll = totalContentHeight > visibleMenuHeight ? (totalContentHeight - visibleMenuHeight).toDouble() : 0.0;
    final menuY = y + height + 2;

    final mx = input.mouseX;
    final my = input.mouseY;
    final isHoveringMenu = _isOpen && mx >= x && mx <= x + width && my >= menuY && my <= menuY + visibleMenuHeight;
    ...
    if (_isOpen) {
      ...
      if (maxScroll > 0) {
        ...
        if (input.isMouseButtonPressed(MouseButtons.left)) {
          if (mx >= scrollbarHitX && mx <= x + width && my >= menuY && my <= menuY + visibleMenuHeight) {
        ...
        if (_isDraggingScrollbar) {
          if (input.isMouseButtonDown(MouseButtons.left)) {
            ...
            final scrollbarHeight = ((visibleMenuHeight / totalContentHeight) * visibleMenuHeight).toInt().clamp(20, visibleMenuHeight);
            final trackSpace = (visibleMenuHeight - scrollbarHeight).toDouble();
            if (trackSpace > 0) {
              final offsetDelta = (deltaY / trackSpace) * maxScroll;
              _scrollOffset = (_dragStartOffset + offsetDelta).clamp(0.0, maxScroll);
            }
          } else {
            _isDraggingScrollbar = false;
          }
      ...
      if (isHoveringMenu && !_isDraggingScrollbar) {
        for (int i = 0; i < options.length; i++) {
          ...
          if (my >= optY && my <= optY + itemHeight && my >= menuY && my <= menuY + visibleMenuHeight) {
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    final currentBorder = _isOpen || _isHovered ? hoverBorderColor : borderColor;
    final currentBg = enabled ? bgColor : const ColorRGBA(16, 20, 26);

    // 1. Dibujar caja gatillo principal
    ctx2d.drawPanel(x, y, width, height, bgColor: currentBg, borderColor: currentBorder);

    // 2. Dibujar texto o placeholder activo
    final currentOpt = selectedOption;
    int textX = x + 12;

    if (currentOpt != null && currentOpt.icon != null && currentOpt.icon!.isNotEmpty) {
      ctx2d.drawText(currentOpt.icon!, textX, y + (height ~/ 4), 16, hoverBorderColor);
      textX += 24;
    }

    if (currentOpt != null) {
      final textColorToUse = enabled ? textColor : const ColorRGBA(110, 120, 135);
      ctx2d.drawText(currentOpt.label, textX, y + (height ~/ 4), 14, textColorToUse);
    } else {
      ctx2d.drawText(placeholder, textX, y + (height ~/ 4), 14, const ColorRGBA(110, 125, 140));
    }

    // 3. Dibujar ícono de flecha desplegable
    final arrowIcon = _isOpen ? Icons.expand_less.glyph : Icons.arrow_drop_down.glyph;
    ctx2d.drawText(arrowIcon, x + width - 26, y + (height ~/ 4), 18, const ColorRGBA(150, 165, 185));
  }
  ...
  void onRenderOverlay(Context2D ctx2d, Context3D ctx3d) {
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      if (isHovered) {
        ctx2d.drawRect(x + 2, itemY + 2, width - 4, itemHeight - 4, hoverItemColor);
      } else if (isSelected) {
        ctx2d.drawRect(x + 2, itemY + 2, width - 4, itemHeight - 4, const ColorRGBA(36, 45, 62));
      }

      int optionTextX = x + 12;
      if (option.icon != null && option.icon!.isNotEmpty) {
        final iconColor = isSelected ? ColorRGBA.accentBlue : const ColorRGBA(160, 175, 195);
        ctx2d.drawText(option.icon!, optionTextX, itemY + 9, 16, iconColor);
        optionTextX += 24;
      }

      final textColorToUse = isSelected ? ColorRGBA.accentBlue : ColorRGBA.white;
      ctx2d.drawText(option.label, optionTextX, itemY + 9, 14, textColorToUse);

      // Marca de selección si está activo
      if (isSelected) {
        ctx2d.drawText(Icons.check.glyph, x + width - 24, itemY + 9, 16, ColorRGBA.accentBlue);
      }
    }
    ...
    if (maxScroll > 0) {
      final scrollbarHeight = ((visibleMenuHeight / totalContentHeight) * visibleMenuHeight).toInt().clamp(20, visibleMenuHeight);
      final scrollbarY = menuY + ((_scrollOffset / maxScroll) * (visibleMenuHeight - scrollbarHeight)).toInt();
      ...
      ctx2d.drawRect(x + width - (barWidth + 2), scrollbarY, barWidth, scrollbarHeight, color);
    }

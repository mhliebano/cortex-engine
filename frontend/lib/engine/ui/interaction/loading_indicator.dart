...
class LoadingIndicator extends Element {
  ...
  LoadingIndicator({
    ...
    this.height = height != 0 ? height : (size > labelFontSize ? size : labelFontSize + 6);
  }
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    if (label != null) {
      final labelX = (variant == LoadingVariant.dots ? (x + size * 2 + 10) : (x + size + 12));
  ...
  void _renderCircular(Context2D ctx2d, int centerY) {
    ...
    _drawThickArc(ctx2d, centerX, centerY, radius, 0, 360, strokeWidth, trackColor);
    ...
    for (int i = 0; i < steps; i++) {
      ...
      final alpha = (math.pow(t2, 1.8) * (color.a - 20) + 20).toInt().clamp(20, 255);
      final stepColor = ColorRGBA(color.r, color.g, color.b, alpha);

      _drawThickArc(ctx2d, centerX, centerY, radius, segStart, segEnd, strokeWidth, stepColor);
    }
    ...
    ctx2d.drawCircle(headX.toInt(), headY.toInt(), capRadius * 0.5, ColorRGBA.white);
  }
  ...
  void _renderSpinner(Context2D ctx2d, int centerY) {
    ...
    for (int i = 0; i < totalDashes; i++) {
      ...
      _drawLineThick(ctx2d, x1, y1, x2, y2, (strokeWidth * 0.9).clamp(2.0, 5.0), dashColor);
    }
  }
  ...
  void _renderDots(Context2D ctx2d, int centerY) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      ctx2d.drawCircle(startX + (i * spacing), centerY + (dotRadius * 0.8).toInt(), dotRadius * 0.7, const ColorRGBA(0, 0, 0, 40));
      // Punto principal
      ctx2d.drawCircle(startX + (i * spacing), centerY - offsetY, dotRadius, dotColor);
      // Brillo en el centro
      ctx2d.drawCircle(startX + (i * spacing), centerY - offsetY, dotRadius * 0.4, ColorRGBA(255, 255, 255, alpha ~/ 2));
    }
  }
  ...
  void _renderPulse(Context2D ctx2d, int centerY) {
    ...
    ctx2d.drawCircleLines(centerX, centerY, pulseR, ColorRGBA(color.r, color.g, color.b, alpha));
  ...
  void _drawThickArc(Context2D ctx2d, int cx, int cy, double radius, double startAngle, double endAngle, double thickness, ColorRGBA col) {
    final half = thickness / 2.0;
    final steps = thickness <= 3 ? 2 : 4;
    for (int i = 0; i < steps; i++) {
      final r = radius - half + (thickness * (i / (steps - 1)));
      ctx2d.drawArcLines(cx, cy, r, startAngle, endAngle, segments: 40, color: col);
    }
  }

  /// Utilidad para dibujar líneas con grosor mediante polígonos rellenos / tiras
  void _drawLineThick(Context2D ctx2d, double x1, double y1, double x2, double y2, double thickness, ColorRGBA col) {
    final dx = x2 - x1;
    final dy = y2 - y1;
    ...
    ctx2d.drawRect((x1 + px).toInt(), (y1 + py).toInt(), (len).toInt(), thickness.toInt(), col);
  }
}

...
class ListView extends Element {
  ...
  ListView({
    ...
    super.marginRight,
    super.marginBottom,
  })  : spacing = spacing ?? (className != null ? Style.merge(className).spacing ?? 4 : 4),
        padding = padding ?? (className != null ? Style.merge(className).padding ?? 6 : 6),
  ...
  void _performLayout() {
    final maxScrollable = _contentHeight > height ? (_contentHeight - height) : 0;
  ...
  void onUpdate(double dt, InputEngine input) {
    ...
    if (isScrollable && maxScroll > 0) {
      ...
      final thumbHeight = (trackHeight * (height / _contentHeight)).clamp(20.0, trackHeight.toDouble());
      final thumbY = trackY + (scrollOffset / maxScroll) * (trackHeight - thumbHeight);

      _isHoveringScrollbar = input.isHovering(scrollbarX - 4, thumbY.toInt(), scrollbarWidth + 8, thumbHeight.toInt());

      if (_isHoveringScrollbar && input.isMouseButtonPressed(MouseButtons.left)) {
      ...
      if (_isDraggingScrollbar) {
        if (input.isMouseButtonDown(MouseButtons.left)) {
          ...
          scrollOffset = (_dragStartOffset + (deltaY * scrollRatio)).clamp(0.0, maxScroll);
          if (controller != null) controller!.offset = scrollOffset;
          _performLayout();
        } else {
    ...
    for (final child in children) {
      final isChildVisibleInView = (child.y + child.height) > y && child.y < (y + height);
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    ctx2d.drawPanel(x, y, width, height, bgColor: bgColor, borderColor: borderColor);
    ...
    if (isScrollable && maxScroll > 0) {
      ...
      final thumbColor = _isDraggingScrollbar
          ...
          : (_isHoveringScrollbar ? const ColorRGBA(110, 130, 155) : const ColorRGBA(60, 72, 90));

      ctx2d.drawRoundRect(scrollbarX, thumbY.toInt(), scrollbarWidth, thumbHeight.toInt(), 0.5, color: thumbColor);
    }
  }

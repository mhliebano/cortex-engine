...
class Card extends Element {
  ...
  Card({
    ...
    super.marginRight,
    super.marginBottom,
  })  : padding = padding ?? (className != null ? Style.merge(className).padding ?? 16 : 16),
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    if (variant == CardVariant.elevated) {
      ...
      ctx2d.drawRoundRect(x + 2, y + shadowOffsetY, width, height, 0.12, color: shadowColor);
    }
    ...
    final activeBg = _isPressed
        ...
        : (_isHovered
            ? ColorRGBA(bgColor.r + 8, bgColor.g + 10, bgColor.b + 12, bgColor.a)
            : bgColor);
    ...
    if (title != null || icon != null) {
      int textX = x + padding;
      if (icon != null) {
        ctx2d.drawText(icon!.glyph, textX, currentY, 18, accentColor ?? const ColorRGBA(41, 128, 185));
        textX += 26;
      }
      if (title != null) {
      ...
      if (badgeText != null) {
        ...
        ctx2d.drawRoundRect(badgeX, currentY - 2, badgeW, 18, 0.5, color: const ColorRGBA(228, 77, 61));
        ctx2d.drawText(badgeText!, badgeX + 6, currentY + 1, 11, ColorRGBA.white);
      }
    ...
    if (subtitle != null) {
      ctx2d.drawText(subtitle!, x + padding, currentY, 12, const ColorRGBA(160, 172, 192));
      currentY += 18;
    }

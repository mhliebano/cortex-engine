...
class SnackBar extends Element {
  ...
  static void renderActiveSnackBar(Context2D ctx2d, Context3D ctx3d, int windowW, int windowH) {
    if (_currentSnackBar == null) return;
    final sb = _currentSnackBar!;
  ...
  void onRenderOverlay(Context2D ctx2d, Context3D ctx3d) {
    ...
    ctx2d.drawRoundRect(x + 2, y + 3, width, height, 0.15, color: const ColorRGBA(0, 0, 0, 140));
    ...
    final closeCol = _isCloseHovered ? ColorRGBA.white : const ColorRGBA(160, 172, 192);

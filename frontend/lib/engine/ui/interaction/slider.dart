...
class Slider extends Element {
  ...
  double get normalizedValue => (max > min) ? ((value - min) / (max - min)).clamp(0.0, 1.0) : 0.0;
  ...
  void _updateValueFromMouseX(double mouseX) {
    ...
    final relativeX = (mouseX - (x + trackPadding)).clamp(0.0, trackW.toDouble());
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    ctx2d.drawRoundRect(trackX, trackY, trackW, trackH, 0.5, color: inactiveTrackColor);

    // 2. Dibujar pista activa rellena a la izquierda del actuador
    final activeW = (normalizedValue * trackW).toInt();
    if (activeW > 0) {
      final activeBg = enabled ? activeTrackColor : const ColorRGBA(60, 90, 120);
      ctx2d.drawRoundRect(trackX, trackY, activeW, trackH, 0.5, color: activeBg);
    }
    ...
    if (divisions != null && divisions! > 0) {
      ...
      for (int i = 0; i <= divisions!; i++) {
        ...
        ctx2d.drawCircle(markX, trackY + (trackH ~/ 2), 1.5, const ColorRGBA(100, 110, 130));
      }
    }
    ...
    if (_isHovered || _isDragging) {
      ctx2d.drawCircle(thumbX, thumbY, thumbRadius + 3.0, const ColorRGBA(41, 128, 185, 80));
    }
    ctx2d.drawCircle(thumbX, thumbY, thumbRadius, enabled ? thumbColor : const ColorRGBA(120, 130, 145));

    // 5. Etiqueta o valor formateado en texto flotante al arrastrar o estático
    final formattedValue = valueFormatter != null
        ? valueFormatter!(value)
        : (divisions != null ? value.toStringAsFixed(0) : value.toStringAsFixed(1));

    if (label != null || _isDragging) {
      final displayTxt = label != null ? "$label: $formattedValue" : formattedValue;
      final txtY = y - 2;
      ctx2d.drawText(displayTxt, trackX, txtY, 12, enabled ? ColorRGBA.white : const ColorRGBA(140, 150, 165));
    }
  }
}

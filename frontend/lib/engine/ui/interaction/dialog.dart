...
import 'package:cortex/engine/ui/interaction/button.dart';
...
class Dialog extends Element {
  ...
  Dialog({
    ...
    int height = 0,
        ...
        _dialogHeight = height != 0 ? height : (child != null ? 160 + child.height : 210),
        super(width: width, height: height != 0 ? height : (child != null ? 160 + child.height : 210)) {
    _confirmBtn = Button(
      label: confirmLabel,
  ...
  static bool get isModalActive => _activeDialog != null && !_activeDialog!._dismissed;

  /// Actualiza la lógica del Diálogo activo.
  static void updateActiveDialog(double dt, InputEngine input, int windowW, int windowH) {
    ...
    if (dlg.dismissibleOutsideClick && input.isMouseButtonPressed(MouseButtons.left)) {
  ...
  static void renderActiveDialog(Context2D ctx2d, Context3D ctx3d, int windowW, int windowH) {
    if (_activeDialog == null || _activeDialog!._dismissed) return;
    final dlg = _activeDialog!;
  ...
  void onRenderOverlay(Context2D ctx2d, Context3D ctx3d) {
    ...
    ctx2d.drawRoundRect(x + 4, y + 6, width, height, 0.1, color: const ColorRGBA(0, 0, 0, 180));
    ...
    ctx2d.drawRect(x + 20, currentY, width - 40, 1, const ColorRGBA(50, 60, 75));
    currentY += 14;

    // 3. Mensaje descriptivo o Contenido personalizado
    if (message != null) {
      ctx2d.drawText(message!, x + 20, currentY, 13, const ColorRGBA(180, 192, 210));
    }

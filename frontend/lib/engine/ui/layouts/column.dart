...
import 'package:cortex/engine/ui/layouts/row.dart';
import 'package:cortex/engine/ui/style.dart';
import 'package:cortex/engine/ui/scroll_controller.dart';
...
class Column extends Element {
  ...
  Column({
    ...
    super.marginRight,
    super.marginBottom,
  })  : spacing = spacing ?? (className != null ? Style.merge(className).spacing ?? 10 : 10),
        padding = padding ?? (className != null ? Style.merge(className).padding ?? 0 : 0),
        _isHeightFixed = height != 0 || (className != null && Style.merge(className).height != null),
        _isWidthFixed = width != 0 || (className != null && Style.merge(className).width != null) {
    ...
    if (isScrollable && !_isHeightFixed && expand == Expand.none && !fillHeight) {
      fillHeight = true;
    }
  ...
  void _performLayout() {
    ...
    final netAvailableHeight = height > (padding * 2) ? height - (padding * 2) : 0;
    ...
    final int contentWidth = maxChildWidth > 0 ? maxChildWidth + (padding * 2) : 0;

    // Ajustar el alto de Column si no especifica alto fijo/expand/isScrollable o mainAxisSize == min
    if (mainAxisSize == MainAxisSize.min || (!_isHeightFixed && !fillHeight && !isScrollable && height == 0)) {
    ...
    final calcAvailableHeight = height > (padding * 2) ? height - (padding * 2) : 0;
    ...
    for (final child in children) {
      ...
      final shouldFillW = child.isFlexWidth || (child is Row && child.mainAxisSize == MainAxisSize.max);
      ...
      switch (crossAxisAlignment) {
        ...
        case CrossAxisAlignment.center:
          if (shouldFillW && availableWidth > 0) {
            ...
            final spaceX = availableWidth > childW ? availableWidth - childW : 0;
  ...
  void onUpdate(double dt, InputEngine input) {
    ...
    if (isScrollable && height > 0) {
      final maxScroll = _contentHeight > height ? (_contentHeight - height).toDouble() : 0.0;

      if (maxScroll > 0) {
        final scrollbarHeight = ((height / _contentHeight) * height).toInt().clamp(20, height);
        final scrollbarY = y + ((scrollOffset / maxScroll) * (height - scrollbarHeight)).toInt();
        ...
        if (input.isMouseButtonPressed(MouseButtons.left)) {
          if (input.isHovering(scrollbarHitX, scrollbarY, 14, scrollbarHeight) || _isHoveringScrollbar) {
        ...
        if (_isDraggingScrollbar) {
          if (input.isMouseButtonDown(MouseButtons.left)) {
            ...
            if (trackSpace > 0) {
              ...
              scrollOffset = (_dragStartOffset + offsetDelta).clamp(0.0, maxScroll);
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    final useClip = (isScrollable || _isHeightFixed || fillHeight || expand == Expand.all || expand == Expand.height) && width > 0 && height > 0;
    if (useClip) {

...
class ToolBar extends Element {
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    for (int i = 0; i < tools.length; i++) {
      ...
      if (tool.isDivider) {
        ctx2d.drawRect(currentX + 4, y + 6, 1, height - 12, const ColorRGBA(50, 58, 75));
        currentX += 10;
        continue;
      }
      ...
      final borderColor = isSelected
          ...
          : (isHovered ? const ColorRGBA(90, 100, 125) : const ColorRGBA(50, 56, 72));
      ctx2d.drawPanel(currentX, y + 4, btnSize, btnSize, bgColor: ColorRGBA.transparent, borderColor: borderColor);
  ...
  void onRenderOverlay(Context2D ctx2d, Context3D ctx3d) {
    ...
    if (_hoveredIndex != null && _hoveredIndex! < tools.length) {
      ...
      final btnCenterX = (x + 8 + _hoveredIndex! * (btnSize + 4)) + (btnSize ~/ 2);

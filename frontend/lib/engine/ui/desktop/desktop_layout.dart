...
import 'package:cortex/engine/ui/desktop/menu_bar.dart';
import 'package:cortex/engine/ui/desktop/side_bar.dart';
import 'package:cortex/engine/ui/desktop/status_bar.dart';
import 'package:cortex/engine/ui/desktop/tool_bar.dart';
...
class DesktopLayout extends Element {
  ...
  void _performLayout() {
    ...
    final availableCenterHeight = currentBottomY > currentTopY ? currentBottomY - currentTopY : 0;
    int currentLeftX = x;
    ...
    if (_body != null && _body!.isVisible) {
      ...
      _body!.y = currentTopY;
      final availableBodyWidth = (x + width) > currentLeftX ? (x + width) - currentLeftX : 0;
  ...
  void onUpdate(double dt, InputEngine input) {
    ...
    if (statusBar != null && statusBar!.isVisible) statusBar!.onUpdate(dt, input);
  }
  ...
  void onRenderOverlay(Context2D ctx2d, Context3D ctx3d) {
    ...
    if (sideBar != null && sideBar!.isVisible) sideBar!.onRenderOverlay(ctx2d, ctx3d);
    if (toolBar != null && toolBar!.isVisible) toolBar!.onRenderOverlay(ctx2d, ctx3d);
    if (statusBar != null && statusBar!.isVisible) statusBar!.onRenderOverlay(ctx2d, ctx3d);
    if (menuBar != null && menuBar!.isVisible) menuBar!.onRenderOverlay(ctx2d, ctx3d);
  }

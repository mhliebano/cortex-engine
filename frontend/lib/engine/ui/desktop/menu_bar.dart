...
class MenuBar extends Element {
  ...
  void onUpdate(double dt, InputEngine input) {
    ...
    if (_openMenuIndex != null && _openMenuIndex! < items.length) {
      ...
      if (openItem.hasSubItems) {
        ...
        final isHoveringDropdown = input.isHovering(openX, dropdownY, dropdownWidth, dropdownHeight);
        ...
        } else if (input.isMouseButtonPressed(MouseButtons.left) && _hoveredIndex == null) {
          _openMenuIndex = null;
        }
      }
    }
  ...
  void onRender(Context2D ctx2d, Context3D ctx3d) {
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      if (item.isDivider) {
        ctx2d.drawRect(currentX + (itemWidth ~/ 2), y + 4, 1, height - 8, const ColorRGBA(50, 56, 70));
        currentX += itemWidth + 4;
        continue;
      }
      ...
      if (openItem.hasSubItems) {
        ...
        for (int j = 0; j < subItems.length; j++) {
          ...
          if (subItem.isDivider) {
            ctx2d.drawRect(openX + 8, subY + 3, dropdownWidth - 16, 1, const ColorRGBA(50, 58, 75));
            subY += 8;
            continue;
          }
          ...
          if (isSubHovered) {
            ctx2d.drawRect(openX + 4, subY, dropdownWidth - 8, itemHeight, hoverColor);
          }
          ...
          if (subItem.icon != null) {
            ctx2d.drawText(subItem.icon!.glyph, drawX, textY, fontSize, textColor);
            drawX += fontSize + 6;
          }
